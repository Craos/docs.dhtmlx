...
AutoComplete.prototype = {
	...
	suggest: function(input, timeout) {
		...
		input._in_progress = window.setTimeout(function() {
			self.send('/suite/suggest.php?query=' + encodeURIComponent(input.value), "", function(response) {
	...
	show: function(input, words) {
		...
		for (var i = 0; i < words.length; i++) {
			...
			if (words[i].type == 'none')
				li.innerHTML = "<img src='/suite/media/api/toc/"+words[i].icon+".png' style='float:left; margin-right:10px;'>"+"<a href=\"/suite/"+words[i].name+"\" class=\"label\">" + words[i].label + "</a><a href=\"/suite/"+words[i].name+"\" class=\"short\">" + words[i]["short"] + "</a>";
			else
				li.innerHTML = "<div class=\"icon " + words[i].type + "\"></div><a href=\"/suite/"+words[i].name+"\" class=\"label\">" + words[i].label + "</a><a href=\"/suite/"+words[i].name+"\" class=\"short\">" + words[i]["short"] + "</a>";
